...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        client_name = AccountCharge.objects.values_list("client", flat=True).distinct()
        ...
        for i in client:
            ...
            client_time_charge = AccountCharge.objects.filter(client=i).aggregate(
                totals=Sum("total_time"), total_charge=Sum("client_total_charge"),
            ...
            if client_balance:
                AccountBalance.objects.filter(client=i).update(
                    ...
                    account_balance=client_payment["total_payment"] - client_time_charge["total_due"]
                )
                account_balance_deficit = AccountBalance.objects.filter(account_balance__lte=0.00)
            else:
