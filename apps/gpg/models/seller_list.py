...
class SellerList(TimeStamped):
    ...
    seller_lead_name = models.CharField(max_length=250, blank=True)
    phone_number = models.CharField(max_length=250)
    email = models.CharField(max_length=250, blank=True)
    lead_status = models.CharField(
        max_length=50, choices=LeadStatus.choices, blank=True
    )
    seller_asking_price = models.CharField(max_length=250, blank=True)
    counter_offer_amount = models.ManyToManyField(
        "gpg.CounterOffer", related_name="seller_counter_offers", blank=True
    )
...
class CounterOffer(TimeStamped):
